...
<c:forEach items='${data.keySet()}' var="aprop">
    <label class="${c1name}"><a data-label="${aprop.getLabel() }" data-comment="${aprop.getComment()}"
                                href="${aprop.getPropertyUrl()}"><c:choose>
    ...
    <div class="${c2name} value">
        <c:forEach items='${data.get(aprop)}' var="ela">
            <div class="toOneLine">
                <a title="&lt;${ela.getValue()}&gt;"
                   class="<c:if test="${results.getLinking().contains(ela.getUrl())}">linkingElement</c:if> <c:if test="${ela.isLocal()}">isLocal</c:if>"
                   href="${ela.getUrl()}<c:if test='${ela.isLocal()}'></c:if>"
                   <c:if test="${!ela.isLocal()}">target="_blank" </c:if>> <c:choose>
